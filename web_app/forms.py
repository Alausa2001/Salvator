...
class BiodataForm(FlaskForm):
    ...
    lastname = StringField(label='Last name/Surname',
                           validators=[Length(min=2), DataRequired()])
    age = IntegerField(label='Age', validators=[
                       NumberRange(min=0, max=120), DataRequired()])
